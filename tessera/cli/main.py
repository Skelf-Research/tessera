...
Examples:
  ...
  tessera identity register --name "My Name"
    ...
    start_parser.add_argument(
        "--peer", action="append", default=[], help="Peer (format: id@host:port)"
    )
    start_parser.add_argument(
        "--bootstrap",
        action="append",
    ...
    sub_parser.add_argument(
        "--commitment", "-c", required=True, help="Commitment (hex)"
    )
    ...
    broadcast_parser.add_argument(
        "--bucket", "-b", type=int, required=True, help="Bucket"
    )
    broadcast_parser.add_argument(
        "--fingerprint", "-f", required=True, help="Fingerprint"
    )
    ...
    # proof commitment
    commit_parser = proof_subparsers.add_parser(
        "commitment", help="Generate commitment"
    )
...
async def node_start(args):
    ...
    node = AsyncDecentralizedNode(
        node_id=args.id, node_type=node_type, data_dir=args.data_dir
    )
    ...
    if args.api_port:
        ...
        api_task = asyncio.create_task(
            run_embedded_api(
                node, args.host, args.api_port, push_service, commitment_storage
            )
        )
        ...
        async with websockets.connect(uri) as ws:
            ...
            if data.get("type") == "peers_response":
                ...
                if not peers:
                    ...
                    for p in peers:
                        print(
                            f"  {p.get('peer_id')} - {p.get('host')}:{p.get('port')} ({p.get('node_type')})"
                        )
    except Exception as e:
        print(f"Error: {e}")
...
def handle_proof_mode(args):
    ...
    if args.proof_command == "subscribe":
        ...
        print(
            "Usage: tessera proof <subscribe|broadcast|fetch|watch|commitment|generate>"
        )
        ...
        async with websockets.connect(uri) as ws:
            while True:
                ...
                for p in data.get("proofs", []):
                    ts = time.strftime(
                        "%H:%M:%S", time.localtime(p.get("timestamp", 0))
                    )
                    print(
                        f"[{ts}] Bucket {p.get('bucket')} - {p.get('org_hint', 'none')}"
                    )
