...
class CommitmentManager:
    ...
    def generate_reception_commitment(
        self, recipient_public_key: bytes, session_id: str
    ) -> bytes:
        """
        Generate a reception commitment for a recipient.
        ...
        # Create commitment: H(recipient_public_key || ephemeral_key || session_id)
        commitment_data = (
            recipient_public_key + ephemeral_key + session_id.encode("utf-8")
        )
    ...
    def cleanup_expired_commitments(self, expiration_time: int = 3600):
        ...
        expired_commitments = [
            commitment
            for commitment, info in self.commitments.items()
