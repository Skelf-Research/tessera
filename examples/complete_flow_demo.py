...
def demo_complete_flow():
    ...
    # Step 1: Verifier sets up reception commitment (happens before deliveries)
    ...
    # In a real system, this commitment would be shared with potential senders
    # For this demo, we'll simulate that by using the same commitment for senders

    # Step 2: Senders make deliveries using the commitment
    print("\n2. Making deliveries...")
    senders = []
    encrypted_proofs = []
    ...
    print("\n3. Verifier checking for deliveries...")
    ...
    # Results
    print("\n5. Results:")
    print(f"   - {len(encrypted_proofs)} deliveries made")
    print(f"   - {len(relevant_proofs)} proofs received by verifier")
    print(f"   - {valid_deliveries} deliveries successfully verified")
...
def demo_privacy_protection():
    ...
    proof = sender.generate_call_proof(metadata)
    encrypted_proof1 = sender.encrypt_proof_for_recipient(proof, commitment1, metadata)
    ...
    print("   - Any metadata about the delivery")
