...
class TesseraService:
    ...
    def setup_routes(self):
        ...
        self.app.route("/commitments/register", methods=["POST"])(
            self.register_commitment
        )
        self.app.route("/commitments/lookup/<commitment_id>", methods=["GET"])(
            self.lookup_commitment
        )
        ...
        self.app.route("/customers/<customer_id>/devices", methods=["POST"])(
            self.register_device
        )
        self.app.route("/customers/<customer_id>/devices", methods=["GET"])(
            self.get_customer_devices
        )
        self.app.route(
            "/customers/<customer_id>/devices/<device_id>", methods=["DELETE"]
        )(self.unregister_device)
        self.app.route("/customers/<customer_id>/link", methods=["POST"])(
            self.link_organization
        )
        self.app.route("/customers/<customer_id>/unlink/<org_id>", methods=["DELETE"])(
            self.unlink_organization
        )
        self.app.route("/customers/<customer_id>/commitments", methods=["GET"])(
            self.get_customer_commitments
        )

        # Organization endpoints
        self.app.route("/organizations/register", methods=["POST"])(
            self.register_organization
        )
        self.app.route(
            "/organizations/<org_id>/customers/<customer_id>/commitments",
            methods=["GET"],
        )(self.org_get_customer_commitments)
    ...
    def broadcast_proof(self):
        ...
        try:
            ...
            if not data or "proof" not in data:
                return (
                    jsonify({"status": "error", "message": "Missing proof data"}),
                    400,
                )
            ...
            if recipients and not commitments:
                for recipient_id in recipients:
                    recipient_commitments = (
                        self.customer_manager.get_customer_commitments(recipient_id)
                    )
            ...
            return (
                jsonify({"status": "error", "message": f"Broadcast failed: {str(e)}"}),
                500,
            )
    ...
    def register_commitment(self):
        ...
        try:
            ...
            if not data or "session_id" not in data:
                return (
                    jsonify({"status": "error", "message": "Missing session_id"}),
                    400,
                )
    ...
    def lookup_commitment(self, commitment_id: str):
        ...
        try:
            if commitment_id not in self.active_commitments:
                return (
                    jsonify({"status": "not_found", "message": "Commitment not found"}),
                    404,
                )
            ...
            if int(time.time()) > commitment_data["expires_at"]:
                ...
                return (
                    jsonify({"status": "expired", "message": "Commitment has expired"}),
                    410,
                )
            ...
            return (
                jsonify({"status": "error", "message": f"Lookup failed: {str(e)}"}),
                500,
            )
    ...
    def get_stats(self):
        ...
        try:
            ...
            recent_verifications = sum(
                1
                for proof in self.verified_proofs
            ...
            expired_commitments = [
                cid
                for cid, data in self.active_commitments.items()
            ...
            return (
                jsonify(
                    {"status": "error", "message": f"Stats retrieval failed: {str(e)}"}
                ),
                500,
            )
    ...
    def register_customer(self):
        ...
        try:
            ...
            if not data or "customer_id" not in data:
                return (
                    jsonify({"status": "error", "message": "Missing customer_id"}),
                    400,
                )
    ...
    def register_device(self, customer_id: str):
        ...
        try:
            ...
            if not data:
                return (
                    jsonify({"status": "error", "message": "Missing device data"}),
                    400,
                )
    ...
    def get_customer_devices(self, customer_id: str):
        ...
        try:
            ...
            if not customer:
                return (
                    jsonify({"status": "not_found", "message": "Customer not found"}),
                    404,
                )
            ...
            return (
                jsonify(
                    {"status": "error", "message": f"Failed to get devices: {str(e)}"}
                ),
                500,
            )
        ...
        try:
            ...
            if not commitments:
                ...
                if not customer:
                    return (
                        jsonify(
                            {"status": "not_found", "message": "Customer not found"}
                        ),
                        404,
                    )
    ...
    def register_organization(self):
        ...
        try:
            ...
            if not data or "organization_id" not in data:
                return (
                    jsonify({"status": "error", "message": "Missing organization_id"}),
                    400,
                )
    ...
    def org_get_customer_commitments(self, org_id: str, customer_id: str):
        ...
        try:
            ...
            if org_id not in self.organizations:
                return (
                    jsonify({"status": "error", "message": "Organization not found"}),
                    404,
                )
