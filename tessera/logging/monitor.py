...
class SecurityMonitor:
    ...
    def __init__(self, security_logger: SecurityLogger, metrics_collector: MetricsCollector):
    ...
    def track_authentication_attempt(self, identity_id: str, success: bool,
    ...
    def track_verification_failure(self, verifier_id: str, metadata: Dict[str, Any] = None):
    ...
    def track_request_pattern(self, endpoint: str, remote_addr: str,
                            user_agent: str = None):
    ...
    def _check_auth_failure_pattern(self, identity_id: str, remote_addr: str):
        ...
        recent_failures = [
            attempt for attempt in self.auth_attempts[identity_id]
            if not attempt['success'] and
               time.time() - attempt['timestamp'] < 60
        ]
    ...
    def _check_verification_failure_pattern(self):
        ...
        recent_failures = [
            failure for failure in self.failed_verifications
            if time.time() - failure['timestamp'] < 60
        ]

        if len(recent_failures) >= self.thresholds['max_verification_failures_per_minute']:
            self.security_logger.log_security_violation(
    ...
    def _check_request_pattern_anomaly(self, pattern_key: str, remote_addr: str):
        ...
        if len(recent_requests) > 10:
            user_agents = [req.get('user_agent') for req in recent_requests if req.get('user_agent')]
    ...
    def get_security_status(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            recent_auth_failures = sum(
                len([a for a in attempts if not a['success'] and current_time - a['timestamp'] < 300])
                for attempts in self.auth_attempts.values()
            )

            recent_verification_failures = len([
                f for f in self.failed_verifications
            ...
            return {
                ...
                'security_violations_24h': len(self.security_logger.get_recent_events('security_violation', 24)),
...
class PerformanceMonitor:
    ...
    def create_alert(self, severity: str, message: str, source: str,
        ...
        with self._lock:
            ...
            self.logger.warning(f"Alert created: {message}", alert_id=alert_id, severity=severity)
            return alert_id
    ...
    def _check_thresholds(self):
        ...
        for threshold in self.thresholds:
            try:
                value = self._extract_metric_value(metrics_summary, threshold.metric_name)
                if value is None:
                    continue
                ...
                if threshold_breached:
                    ...
                    existing_alert = any(
                        alert.source == f"threshold_{threshold.metric_name}" and not alert.acknowledged
                        for alert in self.active_alerts
                    )
                    ...
                    if not existing_alert:
                        self.create_alert(
                            ...
                            {'metric_name': threshold.metric_name, 'current_value': value, 'threshold_value': threshold.value}
                        )

            except Exception as e:
                self.logger.error(f"Error checking threshold {threshold.metric_name}: {e}")

    def _extract_metric_value(self, metrics_summary: Dict[str, Any], metric_path: str) -> Optional[float]:
