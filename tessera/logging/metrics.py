...
class MetricsCollector:
    ...
    def record_proof_generation(self, duration: float, success: bool = True):
        ...
        with self._lock:
            ...
            self._record_time_series('proof_generation', {
                'timestamp': time.time(),
                'duration': duration,
                'success': success
            })
    ...
    def record_proof_verification(self, duration: float, success: bool):
        ...
        with self._lock:
            ...
            self._record_time_series('proof_verification', {
                'timestamp': time.time(),
                'duration': duration,
                'success': success
            })

    def record_network_request(self, duration: float, success: bool, bytes_transferred: int = 0):
        ...
        with self._lock:
            ...
            self._record_time_series('network_request', {
                'timestamp': time.time(),
                'duration': duration,
                'success': success,
    ...
    def record_key_operation(self, operation: str, duration: float, success: bool = True):
        ...
        with self._lock:
            if operation == 'generation' and success:
                ...
                self._update_average(
                    ...
                    lambda avg: setattr(self.key_metrics, 'avg_key_generation_time', avg)
                )
            ...
            self._record_time_series(f'key_{operation}', {
                'timestamp': time.time(),
                'duration': duration,
                'success': success
            })
            ...
            return {
                'proof_metrics': {
                    ...
                    'verification_success_rate': (
                        self.proof_metrics.successful_verifications / max(1, self.proof_metrics.total_verified)
                    ),
                ...
                'network_metrics': {
                    ...
                    'success_rate': (
                        self.network_metrics.successful_requests / max(1, self.network_metrics.total_requests)
                    ),
    ...
    def get_time_series(self, metric_name: str, hours: int = 24) -> List[Dict[str, Any]]:
        ...
        with self._lock:
            ...
            return [
                data for data in self.time_series[metric_name]
    ...
    def get_performance_report(self, hours: int = 24) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            proof_gen_data = [
                d for d in self.time_series.get('proof_generation', [])
                if d.get('timestamp', 0) > cutoff_time and d.get('success')
            ]
            proof_ver_data = [
                d for d in self.time_series.get('proof_verification', [])
                if d.get('timestamp', 0) > cutoff_time
            ]

            # Analyze network performance
            network_data = [
                d for d in self.time_series.get('network_request', [])
                if d.get('timestamp', 0) > cutoff_time
            ]
            ...
            report = {
                ...
                'proof_performance': {
                    ...
                    'avg_generation_time': sum(d.get('duration', 0) for d in proof_gen_data) / max(1, len(proof_gen_data)),
                    'total_verifications': len(proof_ver_data),
                    'successful_verifications': len([d for d in proof_ver_data if d.get('success')]),
                    'verification_success_rate': len([d for d in proof_ver_data if d.get('success')]) / max(1, len(proof_ver_data))
                },
                'network_performance': {
                    'total_requests': len(network_data),
                    'successful_requests': len([d for d in network_data if d.get('success')]),
                    'avg_response_time': sum(d.get('duration', 0) for d in network_data) / max(1, len(network_data)),
    ...
    def _load_metrics(self):
        ...
        try:
            ...
            if 'proof_metrics' in metrics_data:
                ...
                self.proof_metrics.total_generated = proof_data.get('total_generated', 0)
                self.proof_metrics.total_verified = proof_data.get('total_verified', 0)
                self.proof_metrics.successful_verifications = proof_data.get('successful_verifications', 0)
                self.proof_metrics.failed_verifications = proof_data.get('failed_verifications', 0)
                self.proof_metrics.avg_generation_time = proof_data.get('avg_generation_time', 0.0)
                self.proof_metrics.avg_verification_time = proof_data.get('avg_verification_time', 0.0)

            if 'network_metrics' in metrics_data:
                network_data = metrics_data['network_metrics']
                self.network_metrics.total_requests = network_data.get('total_requests', 0)
                self.network_metrics.successful_requests = network_data.get('successful_requests', 0)
                self.network_metrics.failed_requests = network_data.get('failed_requests', 0)
                self.network_metrics.avg_response_time = network_data.get('avg_response_time', 0.0)
                self.network_metrics.bandwidth_bytes = network_data.get('bandwidth_bytes', 0)

            if 'key_metrics' in metrics_data:
                key_data = metrics_data['key_metrics']
                self.key_metrics.total_keys_generated = key_data.get('total_keys_generated', 0)
                self.key_metrics.total_keys_rotated = key_data.get('total_keys_rotated', 0)
                self.key_metrics.total_backups_created = key_data.get('total_backups_created', 0)
                self.key_metrics.avg_key_generation_time = key_data.get('avg_key_generation_time', 0.0)
