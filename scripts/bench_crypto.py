#!/usr/bin/env python3
"""
E1 — Cryptographic microbenchmark for CallDNS.
...
By repo convention (see ../calldns-paper), this harness lives in calldns/ and
...
from calldns.crypto.crypto_utils import CryptoUtils, ZKProver, ZKVerifier, SecureEncryption
...
def print_table(results):
    print("\nCallDNS cryptographic microbenchmark (E1)")
    print("=" * 78)
...
def latex_table(results):
    lines = [
        ...
        "\\begin{table}[t]",
        "\\centering",
        "\\caption{CallDNS cryptographic cost (SECP256k1, single core).}",
...
def main():
    ap = argparse.ArgumentParser(description="CallDNS crypto microbenchmark (E1)")
