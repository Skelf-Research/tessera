...
Examples:
  ...
  tessera-proof watch --subscriber-id device-123 --node localhost:8100
    ...
    sub_parser.add_argument(
        "--commitment", "-c", required=True, help="Commitment hash (hex)"
    )
    sub_parser.add_argument(
        "--subscriber-id", "-s", help="Subscriber ID (default: commitment[:16])"
    )
    sub_parser.add_argument(
        "--org-hints", nargs="*", default=[], help="Organization IDs to filter by"
    )
    sub_parser.add_argument(
        "--time-window",
        type=int,
    ...
    unsub_parser.add_argument(
        "--subscriber-id", "-s", required=True, help="Subscriber ID"
    )
    unsub_parser.add_argument("--node", default="localhost:8100", help="Node address")

    # Broadcast command
    broadcast_parser = subparsers.add_parser("broadcast", help="Broadcast a proof")
    broadcast_parser.add_argument(
        "--bucket", "-b", type=int, required=True, help="Target bucket (0-63)"
    )
    broadcast_parser.add_argument(
        "--fingerprint", "-f", required=True, help="Bloom fingerprint (hex)"
    )
    broadcast_parser.add_argument(
        "--ciphertext", required=True, help="Encrypted proof data (hex)"
    )
    ...
    broadcast_parser.add_argument(
        "--decoys", type=int, default=3, help="Number of decoy proofs (default: 3)"
    )
    broadcast_parser.add_argument(
        "--node", default="localhost:8100", help="Node address"
    )

    # Generate command
    gen_parser = subparsers.add_parser("generate", help="Generate a test proof")
    gen_parser.add_argument(
        "--bucket", "-b", type=int, default=0, help="Target bucket (default: 0)"
    )
    gen_parser.add_argument("--org-hint", help="Organization hint")
    gen_parser.add_argument(
        "--output", "-o", choices=["json", "hex"], default="json", help="Output format"
    )
    ...
    fetch_parser.add_argument("--node", default="localhost:8100", help="Node address")
    ...
    watch_parser.add_argument("--node", default="localhost:8100", help="Node address")
...
async def handle_unsubscribe(args):
    ...
    try:
        async with websockets.connect(uri) as ws:
            await ws.send(
                json.dumps({"type": "unsubscribe", "subscriber_id": args.subscriber_id})
            )
    ...
    try:
        async with websockets.connect(uri) as ws:
            ...
            if args.decoys > 0:
                ...
                print(
                    f"Broadcast {args.decoys} decoy proofs to buckets: {decoy_buckets}"
                )
...
async def handle_fetch(args):
    ...
    try:
        async with websockets.connect(uri) as ws:
            await ws.send(
                json.dumps({"type": "get_proofs", "subscriber_id": args.subscriber_id})
            )
    ...
    try:
        async with websockets.connect(uri) as ws:
            while True:
                ...
                await ws.send(
                    json.dumps(
                        {"type": "get_proofs", "subscriber_id": args.subscriber_id}
                    )
                )
                ...
                if data.get("type") == "proofs":
                    ...
                    for proof in proofs:
                        timestamp = time.strftime(
                            "%H:%M:%S", time.localtime(proof.get("timestamp", 0))
                        )
                        print(
                            f"[{timestamp}] Proof received - Bucket: {proof.get('bucket')}, "
