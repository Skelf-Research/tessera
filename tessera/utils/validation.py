...
class InputValidator:
    ...
    def validate_phone_number(phone: str) -> str:
        ...
        if not InputValidator.PHONE_REGEX.match(phone):
            raise ValidationError(
                "Invalid phone number format (expected E.164 format)", "phone", phone
            )
    ...
    def validate_public_key(public_key: Union[str, bytes]) -> bytes:
        ...
        if isinstance(public_key, str):
            if not InputValidator.HEX_REGEX.match(public_key):
                raise ValidationError(
                    "Public key must be valid hex", "public_key", public_key
                )
            ...
            try:
                ...
                raise ValidationError(
                    "Invalid hex format for public key", "public_key", public_key
                )

        if not isinstance(public_key, bytes):
            raise ValidationError(
                "Public key must be bytes or hex string", "public_key", type(public_key)
            )
    ...
    def validate_session_id(session_id: str) -> str:
        ...
        if not isinstance(session_id, str):
            raise ValidationError(
                "Session ID must be a string", "session_id", session_id
            )
    ...
    def validate_base64(data: str, field_name: str = "data") -> bytes:
        ...
        if not InputValidator.BASE64_REGEX.match(data):
            raise ValidationError(
                f"Invalid base64 format for {field_name}", field_name, data
            )
        ...
        try:
            ...
            raise ValidationError(
                f"Failed to decode base64 {field_name}: {e}", field_name, data
            )
    ...
    def validate_proof_structure(proof: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for field in required_fields:
            if field not in proof:
                raise ValidationError(
                    f"Missing required proof field: {field}", field, None
                )
        ...
        if isinstance(proof["R"], str):
            ...
            raise ValidationError(
                "Proof R must be bytes or base64 string", "R", type(proof["R"])
            )
        ...
        if proof["s"] <= 0 or proof["s"] >= 2**256:
            raise ValidationError(
                "Proof s must be positive and less than 2^256", "s", proof["s"]
            )

        # Validate public key
        if isinstance(proof["public_key"], str):
            proof["public_key"] = InputValidator.validate_base64(
    ...
    def validate_metadata(metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not isinstance(metadata, dict):
            raise ValidationError(
                "Metadata must be a dictionary", "metadata", type(metadata)
            )
        ...
        if "timestamp" in metadata:
            ...
            if not isinstance(timestamp, (int, float)):
                raise ValidationError(
                    "Timestamp must be a number", "timestamp", timestamp
                )

            if timestamp < 0 or timestamp > 2**32:
                raise ValidationError(
                    "Timestamp out of valid range", "timestamp", timestamp
                )
        ...
        if "message" in metadata:
            ...
            if len(message) > 1000:  # Reasonable limit
                raise ValidationError(
                    "Message too long (max 1000 characters)", "message", len(message)
                )
    ...
    def validate_commitment_id(commitment_id: str) -> str:
        ...
        if not isinstance(commitment_id, str):
            raise ValidationError(
                "Commitment ID must be a string", "commitment_id", commitment_id
            )

        if not InputValidator.HEX_REGEX.match(commitment_id):
            raise ValidationError(
                "Commitment ID must be hex format", "commitment_id", commitment_id
            )
    ...
    @staticmethod
    def sanitize_string(
        value: str, max_length: int = 1000, field_name: str = "string"
    ) -> str:
        """
    ...
    def validate_recipients_list(recipients: List[str]) -> List[str]:
        ...
        if not isinstance(recipients, list):
            raise ValidationError(
                "Recipients must be a list", "recipients", type(recipients)
            )

        if len(recipients) > 100:  # Reasonable limit
            raise ValidationError(
                "Too many recipients (max 100)", "recipients", len(recipients)
            )
        ...
        for i, recipient in enumerate(recipients):
            ...
            recipient = InputValidator.sanitize_string(
                recipient, 200, f"recipients[{i}]"
            )
